...
/**
 * InteractionManager — the per-scene orchestrator for hold-E interactions.
 *
 * Owns the E key, the single shared world-anchored icon, and a registry of
 * Interactable targets. Each frame it: (1) picks the closest in-range,
 * canInteract() target; (2) resets hold progress on target change; (3) advances
 * the hold timer while E is down and the player isn't in a locked action;
 * (4) dispatches onInteract() exactly once per press on completion; and (5)
 * drives the icon's position, fade, and progress ring. Two latches enforce the
 * once-per-press contract: the current-target switch clears partial progress,
 * and the `triggered` latch (cleared only on E release) blocks a held key from
 * re-firing on the next target walked into.
 *
 * Inputs:  scene + a structural player query (position + isInteractionBlocked);
 *          registered Interactable targets; per-frame player position and delta.
 * Outputs: fires targets' onInteract(); drives the shared InteractionIcon.
 * @calledby the gameplay scene — built/torn down with the world (HMR-safe) and
 *           ticked each frame after the player resolves, before the HUD repaints.
 * @calls    each Interactable's range/anchor/canInteract/onInteract surface and
 *           the icon's position/fade/progress drawing.
 */
...
export class InteractionManager {
  ...
  // binds E and creates the shared icon; throws if the scene has no keyboard
  ...
  // per-frame tick: picks closest target, runs hold timer, fires onInteract once per press, fades icon
  ...
  // Tears down the key binding, icon, and target registry (world rebuild / HMR).
  destroy(): void {
  ...
  // linear scan returning the nearest canInteract() target within its own range, or null
